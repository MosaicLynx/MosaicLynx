---
name: design-author
description: MosaicLynx の承認済み要件・既存設計判断を、責務境界、コンポーネント、依存方向、trust boundary、主要フロー、データ所有、運用前提、検証方針を含む基本設計へ整理する。設計判断は docs/design に統合し、API・wire format・暗号パラメータ・実装コードは必要以上に決めない。
---
...
- Extension、SDK、Relay、chain adapter、backup / protocol package、外部 wallet-core の責務と依存方向
...
- TypeScript の具体的な関数、module、package、database、UI の実装詳細
...
秘密情報を設計例、図、ログ、エラー、fixtureへ含めない。外部 wallet-core と Binding の境界を弱めず、MosaicLynx の SDK、Extension、Relay、chain adapter または Binding へ鍵管理・暗号・導出・署名意味判断を移さない。
...
自己確認では、Extension / SDK / Relay / chain adapter / 外部 wallet-core の境界、Symbol / NEM、Mainnet / Testnet、秘密情報の所有・保持・破棄、設計判断と仕様の分離、既存資料との競合を重点的に確認する。
