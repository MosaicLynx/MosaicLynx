...
5. `docs/concept/` の既存コンセプトおよび対象へ直接必要な正式上流資料

## 対象と出力

- ユーザーが出力先を指定した場合はそのパスを使う。
- 未指定の場合は対象を確定し、`docs/concept/<topic>.md` に新規作成する。
- このリポジトリは Extension、SDK、Relay、chain adapter、backup / protocol package を含む monorepo である。対象範囲が不明な場合は出力先を推測しない。
...
- 詳細な実装資料や外部 wallet-core 資料は原則として参照しない。コンセプトの成立性や責任境界の確認に不可欠な場合だけ補助的に使う。
- Symbol / NEM、Mainnet / Testnet、Extension / SDK / Relay、MosaicLynx Application / 外部 wallet-core の区別は明示資料にある境界を保つ。存在しない app、package、binding を実装済みと記載しない。
