...
description: MosaicLynx の正式資料から、任意のフェーズ用に非規範的な Phase Context を評価・作成・refresh する。Context の必要性を反復参照コストで判定し、正式資料やレビュー成果物は変更しない。
---
...
および `last_refreshed` を記録する。登録パスは、`AGENTS.md` に追加した実在するリポジトリ相対パスと
する。`AGENTS.md` に登録する前に Context ファイルだけを作成しない。登録、frontmatter、source map、
見出しおよびリンクを作成後に確認する。
