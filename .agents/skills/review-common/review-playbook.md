...
- 対象は MosaicLynx の `apps/*`、`packages/*`、ドキュメント、テスト、fixture、release evidence である。
- `symbol-nem-wallet-core` と固定済み Native / WASM Binding は MosaicLynx から利用する外部契約であり、root repository の実装対象ではない。`_snwc` を対象にする場合は、そのディレクトリの repository instructions を別途適用する。
- 作業指針はリポジトリルートの `AGENTS.md` を読む。技術的な正本は対象機能の要件、仕様、`docs/design/`、適用可能な ADR、必要な公式資料である。
- 主要な資料置き場は `docs/concept/`、`docs/requirements/`、`docs/design/`、`docs/specifications/`、レビュー成果物は `docs/reviews/` である。設計と設計判断は `docs/design/` に統合する。
- `Symbol / NEM`、`Mainnet / Testnet`、Extension / SDK / Relay、wallet-core / Application、protocol / SDK を混同しない。存在しない package、app、binding を実装済みと扱わない。
...
`## 検証` と対象 package / app の script を適用する。レビューのフェーズ
だけで検証範囲を決めず、変更分類または明示された依頼範囲に該当するものだけ実行する。

docs-only では文書・リンク・traceability・整合性の確認を行い、agent / skill-only では
Skill の構造・参照・Markdown・必要な validator を確認する。いずれも app / package の実装
テストを自動実行しない。Concept / Requirements / Design /
