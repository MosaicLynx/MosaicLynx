...
この checklist は、MosaicLynx の Extension、SDK、Relay、chain adapter、backup / protocol package と、外部 `symbol-nem-wallet-core` を含む Requirements Review で、既存の Concept、ユーザー要求、Requirements、または製品が明示的に扱う protected asset / responsibility に対応する security property の欠落を確認するためのレビュー観点である。
...
- SDK、Extension、Relay、外部 wallet-core / Binding へ不要な秘密情報を渡さないこと
...
特に、鍵管理、暗号処理、導出、署名は外部 wallet-core の責任、Account 選択・承認・Provider境界・Relay中継は MosaicLynx 側の責任として、意図しない別レイヤへ流れていないか確認する。Design レベルのコンポーネント構造や依存方向そのものは要求しない。
