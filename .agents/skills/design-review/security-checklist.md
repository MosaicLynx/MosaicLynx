...
この checklist は、MosaicLynx の Extension、SDK、Relay、chain adapter、backup / protocol package と、外部 `symbol-nem-wallet-core` / Binding の境界を含む Design Review で、既存の Concept、Requirements、対象 Design、既存の適用可能な Design Decision、またはユーザー要求に追跡できる security architecture の欠落を探索するためのレビュー観点である。
...
- Browser Extension / privileged UI
- SDK / Provider / dApp caller
- Relay / transport
- chain adapter / protocol package
- external wallet-core / Native C ABI / WASM Binding（対象に含む場合）
- caller
- persistent storage
...
Extension / SDK / Relay / external wallet-core / Binding 間で、MosaicLynx の Application 層や Binding が外部 wallet-core の secret ownership を代替したり、下位層の責任が上位へ逆流したりしていないか確認する。
...
MosaicLynx または外部 wallet-core の Native C ABI / WASM Binding が、外部 wallet-core の次の意味と責任を奪っていないか確認する。対象に Binding がない場合は適用外とする。
...
```text
dApp / Provider / Extension UI
              ↓
       SDK / Extension / Relay
              ↓
      external wallet-core
```

- external wallet-core が UI / browser / platform-specific policy に依存していない
- SDK、Extension、Relay、Binding が外部 wallet-core の secret ownership を代替していない
- Application 層が cryptographic state の正本になっていない
- 循環依存や責任逆流がない
...
- zeroize library、TypeScript type、clone / allocation / temporary、unsafe の具体的要求
