...
- app、package、機能が指定され README path がない場合は直接対応する README を
...
- 同じ browser、Extension / Provider、Relay、外部 wallet-core Binding（対象に含む場合）の責任境界と release status
...
README 全体を読んだ後、対象 app / package の `package.json`、workspace 設定、TypeScript declaration、
公開 export、主要実装、仕様、license、テスト、sample、build / packaging script を必要な範囲で照合する。
外部 wallet-core の Native C ABI / WASM Binding は README が実際に説明する場合だけ対象に含める。
確認できない環境や未実行 sample は成功扱いにしない。
...
- 概要、install command、package name、import、Node / browser / Extension 前提
...
- browser / bundler path、Extension / Provider boundary、unsupported target、external Binding の availability（対象に含む場合）
- 外部 wallet-core Binding と SDK / Extension package との責任境界
