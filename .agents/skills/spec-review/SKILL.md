...
- 実装者からの対象一致する仕様フィードバックが `docs/reviews/implementation/` にある場合、またはユーザーが明示した場合だけ補助資料として確認する。固定名のファイルを前提にしない。
...
- Symbol / NEM、Mainnet / Testnet、SDK とプロトコル、Extension / SDK / Relay / 外部 wallet-core、opaque byte 列の区別
...
ここで確認するのは `what exact behavior / contract must be observed` である。TypeScript の function / module、clone / copy、temporary、zeroization の実装、Native C ABI / WASM Binding の内部実装、実際の library call、side-channel の具体実装、parser / fuzz harness の実装、具体的な memory lifetime は Implementation Review へ委譲する。UI 方式や内部 token 方式を指定しない。
...
6. 必要な修正を内部実装方式、library、TypeScript type、memory layout、具体的 parser / fuzz framework に固定せず表現できる。
