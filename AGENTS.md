...
変更があるコミットを作成する場合は、タイトルだけのコミットを作成しない。タイトルおよび本文の説明は日本語で記述する。プレフィックス、ファイルパス、API名、固有名詞、技術用語、コマンド名などは必要な範囲で原表記を使用してよい。タイトルの後に空行を置き、`- ` で始まる変更内容の箇条書きを最低1項目続ける。変更が1点だけでも本文を省略しない。
