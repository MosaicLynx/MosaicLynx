...
MVPの互換対象はSSS API `1.0.4`の次のsubsetに固定する。

| API | MVP | 規則 |
| --- | --- | --- |
| `window.requestSSS()` / `isAllowedSSS()` | 対応 | 実際のOrigin Permissionを返し、固定`true`を返さない |
| `activeName` / `activeAddress` / `activePublicKey` / `activeNetworkType` | 対応 | 許可済みactive Accountの公開情報だけ。未接続は空値で偽装せずerror / undefined |
| `setTransactionByPayload()` | 対応 | hex payloadをstaging。schema解析は`requestSign()`時にも再実行 |
| `setTransaction()` | 条件付き | 固定版symbol-sdkのtransaction objectを即時serializeし、symbol-sdk object自体を保持しない。serialize不能なら拒否 |
| `requestSign()` | 対応 | Product 12.4 allowlist、専用承認、single-use |
| `setMessage()` / `requestSignEncription()` | Legacyのみ対応 | raw bytes、recipient、Origin bindingを専用Legacy画面で確認 |
| `requestSignCosignatureTransaction()` | 条件付き | 完全な親aggregate / multisig payloadが同時にstagingされる場合だけ対応 |
| `requestSignWithCosignatories()` | 非対応 | dApp指定cosignatory集合を信用せず`UNSUPPORTED_TRANSACTION` |
| `getActiveAccountToken()` | 非対応 | token発行や永続署名権限を提供しない |

非対応APIはmethodの不存在またはdocumented errorで安全側に失敗し、成功に見えるplaceholderを返さない。SSS互換testは上表の戻り値、例外、同期getter / Promise methodの差を固定する。
...
```text
┌ LEGACY MESSAGE / レガシーメッセージ       [再利用リスク]
│ xn--… のASCII Origin と Unicode Originを同じ強さで全文表示
│ ! Origin、用途、nonce、有効期限は署名bytesに含まれません。
│   この署名は別の用途で再利用される可能性があります。
├ 実際の署名データ / Exact signing data
│ encoding: hex   byte length   digest   UTF-8 safe view
│ U+202E / zero-width等はcode pointで可視化
└ [署名を拒否]                     [リスクを理解して署名 / Sign with risk]
```
...
| `legacy.warning` | このLegacy署名にはOrigin、用途、nonce、有効期限が含まれず、別の用途で再利用される可能性があります。 | This legacy signature does not include the origin, purpose, nonce, or expiry and may be reused for another purpose. |
...
| Legacy `UX-LEGACY-BIDI-001` | domain separation欠如と再利用riskを説明し、未知要求を拒否 | Bidi/zero-widthを含む未知Legacy requestを承認 |
