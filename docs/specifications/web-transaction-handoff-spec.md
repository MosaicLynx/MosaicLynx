...
| MosaicLynx SDK API version | `1.0.0`                 |
| 必須 Provider API          | `2.x`                   |
...
Relay は current Relay generation context を持つ。generation context は非秘密の opaque context とし、Relay restart、active session state の完全消失、storage loss または既存 state の継続性を保証できなくなった場合に切り替える。切り替え時、旧 generation の pending session は復旧せず、旧 generation 全体を失効させる。active session state は handoff の有効期間に限る bounded / temporary state とし、payload history または ciphertext history を durable long-term storage として保持してはならない。この要件は protocol semantics であり、特定の storage engine、database / cache product、persistence engine、process runtime または deployment topology を選択するものではない。Relay の storage / deployment backend は [Relay Specification の `OPEN-RELAY-002`](./relay.md#open-relay-002-storage-backend--deployment-topology) および applicable lower implementation authority に従う。
...
dApp backendは入力schema、`initiatorOrigin`、TTLを検証し、`SHA-256(UTF8("mosaiclynx.origin.v1\0") || UTF8(JCS(OriginProofInput)))`をEd25519で署名して`OriginProof`を返す。MosaicLynx SDKはresponseをそのまま信頼せずrequestへ含め、Appが独立検証する。
...
MosaicLynx SDKはApp Linkを現在のbrowsing contextから開く。アプリがインストール済みの正常系では新しいbrowser tabを作らない。アプリ未導入時のHTTPS fallbackは正常な署名フローではなく、fallback pageはfragmentをnetwork、log、analyticsへ送らず、URLから直ちに除去して導入案内を表示する。
...
MosaicLynx SDKとAppはsession secretから次の二つのAES keyを導出する。
...
- Originは`https://relay.mosaiclynx.app`、API prefixは`/v1`に固定し、MosaicLynx SDK optionやdApp引数で変更できない。以下のendpoint表記はこのOriginに対する絶対pathである。
...
Handoff の protocol contract は、session state transition、response registration、ACK、cancel、expiry および terminal purge が logical に一貫して成立し、race により duplicate success、terminal-state reuse、cross-session mutation、conflicting response または consumed state revival が発生しないことを要求する。この atomicity は Relay が満たすべき logical requirement であり、特定の storage engine、database / cache product、CAS、lock、queue、broker、script、process runtime または deployment topology を指定しない。Relay の storage / deployment backend の選択は [Relay Specification の `OPEN-RELAY-002`](./relay.md#open-relay-002-storage-backend--deployment-topology) および applicable lower implementation authority に委譲する。

expiry および retention deadline は protocol semantics として一意でなければならず、expiry を超えた state を有効な handoff として扱ってはならない。restart、storage / process / instance / cluster state loss または continuity loss の後に expired / stale state を復活させず、silent recovery を行わない。state continuity を保証できなくなった場合は generation を切り替え、old generation の pending session を current state として復旧しない。retry は fresh generation、request / session identity、transport authorization context、暗号化 envelope および新しい利用者承認を伴う新しい handoff とする。

Relay は `EncryptedRelayEnvelope` と handoff に必要な最小限の非秘密 metadata だけを opaque に扱い、ciphertext を復号せず、plaintext の transaction / message / signed result、session secret または semantic meaning を保持・解析しない。internal storage key、log、diagnostics、backup、analytics または telemetry は raw session identifier、raw IP、credential / token、E2E secret、plaintext または ciphertext 全文を不用意に露出させてはならない。検証用 representation、storage layout および retention / purge の具体方式は Relay Specification と applicable lower implementation authority に従う。active state は bounded / temporary に保ち、terminal purge、long-term payload / ciphertext history の非保持、backup / analytics duplication の禁止および state-loss fail-closed を維持する。
...
MosaicLynx SDKはtransport固有errorを次の共通規則で正規化する。
...
- 応答待機中にOriginが変わるnavigation、page disposal、MosaicLynx SDK cancelが発生した場合はRelayをcancelし、結果を返さない。
...
diagnostics、Relay log、telemetryにpayload、signed payload、hash、public key、Origin、request ID、session ID、token、secret、URL、暗号文を含めない。MosaicLynx SDKはdiagnostics callbackの例外を署名フローへ伝播させない。
