...
- Mainnet signing は [ADR 0001](../adr/0001-mainnet-evidence-lite.md) と release gate を満たす build の capability に限る。gate が未達成または判定不能な場合は Mainnet signing を利用可能と報告しない。
...
| approval                                | 一つの request / signing target への利用者の明示承認と request-specific authentication が結び付いた短寿命状態                                |
...
| ------------------- | ----------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `connect`           | `MosaicScope`（`chain`、`network`）                                                                   | 指定 Scope で許可された Provider Account record の readonly collection。page-facing projection は §10 の公開規則に従う |
| `disconnect`        | なし                                                                                                  | `void`。current caller の connection / permission を終了する                                                           |
| `getAccounts`       | なし                                                                                                  | current caller に公開可能な Account collection                                                                         |
| `getActiveAccount`  | `MosaicScope`                                                                                         | Scope に対応する Account または `undefined`                                                                            |
| `signTransaction`   | Scope、hex payload、既存 contract の optional Account selector                                        | 検証済み `SignedTransaction`（payload、hash、signer public key）                                                       |
| `signMessage`       | Scope、既存 structured message の purpose、nonce、issuedAt、expiry、payload および既存 optional field | 検証済み `SignedMessage`（signature、signer public key、digest、message）                                              |
| `cosignTransaction` | Chain-specific な parent / payload、detached 等、既存 contract の optional selector                   | Chain-specific `MosaicLynxCosignature`。Symbol と NEM の target / result shape を混同しない                            |
...
Provider-native Account record に含まれる selector、display metadata またはその他の field は §10 の page-facing public projection の制約を受ける。内部 selector を利用する場合も、untrusted page input のまま key selection / authorization に用いず、現在の browser caller、permission、Scope、Account および expected signer と照合する。
...
Provider は既存 `ProviderErrorCode` と Handoff / SDK の concrete error authority を使用する。本書は新しい public error code、error taxonomy、alias または error detail を追加しない。Provider API package に定義済みの `USER_REJECTED`、`UNAUTHORIZED_ORIGIN`、`VAULT_LOCKED`、`INVALID_PARAMS`、`INVALID_MESSAGE`、`NONCE_REUSED`、`UNSUPPORTED_CHAIN`、`ACCOUNT_NOT_FOUND`、`UNSUPPORTED_TRANSACTION`、`INVALID_TRANSACTION`、`CHAIN_MISMATCH`、`NETWORK_MISMATCH`、`REQUEST_EXPIRED`、`CONTEXT_CHANGED`、`RESOURCE_LIMIT` および `INTERNAL_ERROR` は、既存 package contract の集合として扱い、意味を拡張しない。
...
Connection permission は、署名要求を trusted inspection、approval、authentication または wallet-core signing へ自動的に通す authority ではない。各 signing request は、current permission に加えて、request-specific な explicit approval と `every-signature` authentication を必要とする。
...
現行 Provider package / Extension Adapter には、privileged host 内で Account を選択するための opaque selector が存在し得る。selector は request routing の内部参照であり、Public Account Identity、ownership proof、permission authority または key material ではない。現在の Provider 型にある `id` / `profileId` と page-facing projection の境界を最終的に分離する方法は `OPEN-BEX-001` とし、本書は内部 ID の page への露出を許可しない。
...
| `AUTHORIZED`     | 当該 request / target への explicit approval と `every-signature` authentication が成立した短寿命状態。署名前再検証が必要 |
| `SIGNING`        | 再検証済み target を wallet-core contract へ渡している状態。自動再実行不可                                                |
| `SUCCEEDED`      | wallet-core result と request、target、signer、Account、Scope、operation の対応を検証済み                                 |
...
Approve は一つの logical signing target に対する single-use authorization である。approval binding は [signing-protocol.md §5.3](./signing-protocol.md) に従い、適用範囲で caller / Origin、document context、requestId、session、operation、permission revision、Profile、Account、Chain / Network、capability、target、inspection result、signer role、expiry および freshness を含む。
...
Profile `UNLOCKED` は signing authorization ではない。connection permission、unlock、過去の authentication、Account cache または trusted UI の表示だけで signing してはならない。Profile / Security authority が要求する `every-signature` authentication を、当該 request の approval と別に確認する。Browser-specific OS biometric API、password prompt 実装、fallback および rate limit は独自に固定しない。
...
- authentication state の request-specific validity と approval binding
- response recipient、Provider instance、current lifecycle / generation context

不一致、stale、missing、expired、revoked、lock、context loss、target mutation または validation failure があれば old approval を使わず、既存 signing protocol の `INVALIDATED`、`EXPIRED`、`FAILED` 等の applicable terminal semantics に従う。対象を自動修正、Account / Network を自動切替、payload を再構成または再承認なしに再署名しない。

### 18.2 wallet-core invocation

wallet-core を呼び出す前に、privileged host が trusted validation、trusted inspection、explicit approval および required authentication を完了する。wallet-core へ Web Application の raw request、page summary、untrusted Account selector または未検証 payload をそのまま渡して signing authority を委譲しない。
...
次の lifecycle event は、request / approval / authentication / signing / response correlation の continuity を再評価する契機である。
...
安全に current caller、permission、target、approval、authentication、Profile / Account、protocol context および response recipient を再構築できない場合、古い authorization を再利用せず `INVALIDATED` または applicable unknown outcome として扱う。再構築できないことを、署名未実行、user rejection または signing failure の証明にしない。
...
- pending request、`AUTHORIZED` approval、authentication success、`SIGNING` state、wallet-core operation
...
restart 前の authorization を安全に再構成できない場合は invalidation する。`SIGNING` 中の process / wallet-core response loss は `RESULT_UNKNOWN` の定義を使用し、同じ target を自動再署名しない。確定済み result の delivery だけが不明なら `SUCCEEDED + DELIVERY_UNKNOWN` として signing outcome と分離する。
...
- `SUCCEEDED` は wallet-core result と target の検証が完了した signing outcome である。page に response が届いたことだけで `SUCCEEDED` としない。
- `RESULT_UNKNOWN` は signing が実行されたか、成功したかを安全に確定できない状態である。自動 re-sign、未署名への変換、user rejection への変換をしない。
- `SUCCEEDED + DELIVERY_UNKNOWN` は signed result が確定したが response delivery が確認できない状態である。再署名せず、既存の delivery / retrieval contract がある場合だけそれに従う。
- Provider method の Promise resolve、transport delivery、event emission、Relay response、HTTP status または page callback success は signing result の cryptographic validity の証明ではない。
...
| signed result delivery failure            | `SUCCEEDED + DELIVERY_UNKNOWN`。signing error / user rejection / 未署名としない                        |
...
- Profile switch、Profile deletion、Profile lock / unlock
- Chain / Network changeまたは Profile scope の変更
- permission revoke、permission scope / Account set / revision change
- vault lock、wallet-core store replacement、security incident

old Account の approval、authentication、key reference、pending result または response を new Account / Profile / Scope へ移さない。signing が既に不可逆な段階に進んでいる場合、permission revoke / disconnect の発生だけから user rejection、未署名または cancellation を推測しない。result unknown は Signing Protocol の semantics を維持する。
...
- official distribution、integrity、dependency、Mainnet release evidence および update / rollback policy は release / security authority へ委譲する。
...
| error の範囲                                                                                                        | authority                                                                                                 |
| ------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| common logical error、validation、protocol、unsupported、user rejection、security rejection、signing error 等の意味 | [interfaces.md](./interfaces.md) と [signing-protocol.md](./signing-protocol.md)                          |
| Provider / SDK / Handoff の concrete public code                                                                    | [web-transaction-handoff-spec.md §10](./web-transaction-handoff-spec.md) および既存 Provider API contract |
| Chain-specific parse / signature / result failure                                                                   | [chain-compatibility-spec.md](./chain-compatibility-spec.md) と chain integration contract                |
| wallet-core internal error / warning / store failure                                                                | wallet-core contract。page-facing concrete detail は Extension / Handoff の既存 mapping                   |
...
error response は signing outcome、approval、authentication、Origin verification、Account ownership または session existence を過剰に推測できる detail を返さない。具体的 status / code mapping は Handoff / Provider contract に従い、Browser Extension が endpoint-specific policy を再定義しない。
...
4. `UNLOCKED` は approval ではなく、authentication は approval の代替ではない。
5. trusted inspection と全 security-relevant field の表示が signing 前に完了する。
6. blind signing、hash-only approval、summary-only approval、Node lookup 補完および raw fallback を許可しない。
7. 一つの approval は一つの logical signing target に対する single-use authorization である。
8. stale、expired、duplicate、replayed、cancelled、revoked、terminal または lifecycle-lost state を再利用しない。
9. wallet-core は trusted validation、inspection、explicit approval、required authentication および pre-sign revalidation の後だけ呼び出す。
10. private key、Mnemonic、Wallet Store plaintext、password、authentication secret、credential および signing internal secret を page / Provider / SDK / Relay へ露出しない。
11. response correlation は requestId だけに依存せず、caller、document、operation、Scope、Account、target および lifecycle context を検証し、失敗時は fail-closed とする。
12. `RESULT_UNKNOWN` の不確実性から automatic re-sign を行わない。`DELIVERY_UNKNOWN` は確定済み result の delivery 状態として分離する。
13. Navigation、tab / frame replacement、Provider replacement、lock、permission revision、Profile / Account change、restart および update 後に stale approval を継続しない。
14. signing outcome、user rejection、cancellation、transport delivery、response receipt、connection success および authentication success を混同しない。
...
5. `Scope`、Profile、Account、expected signer、Chain / Network、capability、expiry、requestId、duplicate / replay および target structure を signing 前に再検証する。
6. `RECEIVED → VALIDATED → INSPECTED → AWAITING_USER → AUTHORIZED → SIGNING → SUCCEEDED` と全 terminal state を Signing Protocol に従って扱い、独自の危険な state を追加しない。
...
9. `AUTHORIZED → SIGNING` 直前に caller、permission revision、Profile / Account、Scope、payload、digest、signer role、expiry、authentication、approval binding および response recipient を再検証する。
10. wallet-core を承認済み・再検証済み target に限って呼び出し、result を元 request、target、signer、Account、Chain / Network および operation と検証する。
...
12. SDK timeout、request expiry、user rejection、explicit cancellation、UI close、lifecycle loss、`RESULT_UNKNOWN` および `DELIVERY_UNKNOWN` を相互に誤変換せず、自動再署名しない。
13. Service Worker / background restart、extension update、browser restart、navigation、tab / frame destruction 後に old approval、authentication、signing state、response correlation または secret を推測復元しない。
14. page-facing error、diagnostics、logging、events、cache および URL に secret、credential、internal ID、stack trace、raw Wallet Store error または不要な full payload を露出しない。
15. Mainnet signing capability が release evidence / gate と一致し、gate 未達成または判定不能の build が signing success を提供しない。

## 32. OPEN Issues

本書で上流未決事項を独自に確定しない。次を Browser Extension の実装開始前または関連下位仕様で解消する。

### OPEN-BEX-001: Provider Account projection と opaque selector

- **問題:** Provider package / Extension Adapter の Account selector と、Interfaces / SDK が禁止する internal Account / Profile ID の page-facing 境界を、現在の型・wire contract 上で最終分離する方法が未確定である。
- **影響:** `getAccounts()`、`getActiveAccount()`、expected signer の選択、`signTransaction()` / `cosignTransaction()` の provider params、public Account disclosure。
- **本書の扱い:** Public Account Identity に internal ID を含めない。selector を ownership、permission、approval または signer authority としない。型分割・opaque handle の concrete shape・SDK / Provider compatibility は独自追加しない。
- **戻すべき上流:** [interfaces.md §5.3、§8](./interfaces.md)、[sdk.md §5、§6](./sdk.md)、[web-transaction-handoff-spec.md §5、§6](./web-transaction-handoff-spec.md)、Provider API contract。
...
| -------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------- |
| `BR-001`、`BR-013`、`CR-011`                             | [Browser Extension Design §3、§19](../design/browser-extension.md)、[Architecture §2、§5](../design/architecture.md)                                              | [product-spec.md](./product-spec.md)、[ADR 0001](../adr/0001-mainnet-evidence-lite.md)                                                           | §2.3、§26、§31.15           |
...
| `SDK-FR-008`、`SDK-FR-010`、`SDK-FR-011`                 | [Signing Flow §20〜§22](../design/signing-flow.md)、[SDK Design §12〜§16](../design/sdk.md)                                                                       | [interfaces.md §6、§10](./interfaces.md)、[signing-protocol.md §5、§19](./signing-protocol.md)、[Handoff §10](./web-transaction-handoff-spec.md) | §22〜§24、§27               |
