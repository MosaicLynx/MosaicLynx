...
利用者が許可していない Origin、Profile、Account、Chain または Network からの要求を、暗黙の接続、許可 scope の拡張または Account、Chain、Network の切り替えによって許可してはならない。接続許可は包括的な署名許可ではなく、接続済みの Origin からの署名も要求ごとに利用者の明示的な承認を必要とする。自動署名または永続的な包括署名許可を導入してはならない。
...
| ------ | ----------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| BR-001 | Concept Sheet 1、6.5、12；共通要件 §3、CR-011                                                                                       | Browser Extension が最初の milestone であり、Chrome を最初の提供形態とすることは Concept Sheet §12 に明記されている。初回 milestone を Chrome のみに限定する判断を本要件定義で正式決定した。BR-AC-013 で外部から確認する。                                                                                                                                                                                                                                |
...
| BR-004 | Concept Sheet 5、11、13；共通要件 CR-005、CR-009、CR-NFR-008；Chrome のページ・frame コンテキスト制約                               | 上流根拠・外部制約をもとに、browser-observed context と Origin の binding、Web Origin / Profile / Account / Chain / Network の接続許可 scope、明示的な作成・変更・撤回および要求ごとの署名承認を本要件定義で正式決定した。初回の HTTPS / loopback HTTP、top-level 限定、拒否対象の範囲も本要件定義で正式決定した。BR-AC-001、BR-AC-004、BR-AC-010、BR-AC-011、BR-AC-012 で外部から確認する。具体的な parsing / canonicalization / permission 設定は下流。 |
...
| BR-013 | Concept Sheet 12、14、15；共通要件 CR-NFR-006                                                                                       | 上流根拠あり。Mainnet gate と gate 未達成時の無効化は Concept Sheet と共通要件に明記されている。gate の評価時点、公開後の evidence 期限切れ・失効・検証不能時の扱い、および build-time / runtime の責任境界は release operation / Mainnet evidence policy へ引き継ぎ、本要件では決定しない。                                                                                                                                                              |
