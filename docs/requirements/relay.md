...
SDK と正規 Mobile App の間の一時的な E2E secret handoff は Relay を経由せず、具体的な handoff 方式は後続仕様へ委ねる。下流仕様が URL fragment 等の一時的な client-side handoff を採用する場合も、安全条件を満たさなければならず、その方式は Relay credential transport ではない。handoff secret は Relay へ送信せず、HTTP request、Referer、server access log、browser storage、diagnostics、analytics、telemetry、error reporting、clipboard またはその他の別経路へ転送してはならない。fallback 経路で必要最小限の一時処理を行う場合も、DOM へ不要に露出させず、処理後に URL または browsing context から除去し、継続保持してはならない。

Relay endpoint authorization credential を署名秘密情報または E2E session secret / derived encryption material と同一視してはならず、いずれも署名能力、利用者承認または Wallet Core の代替として扱ってはならない。具体的な credential の形式、検証方式、保存方式および handoff secret の fragment 表現は後続仕様へ委ねる。
...
| RR-AC-006 | RR-003、RR-008、RR-NFR-004                            | Relay が opaque request / response envelope と必要最小限の metadata だけを扱い、signing secret を受信・処理せず、E2E session secret / derived encryption material を受信・復号・保持・hash 化・導出せず、E2E envelope を復号できない。Relay endpoint authorization credential は必要最小限だけを扱い、raw 値を log、diagnostics、error、analytics、telemetry、persistent history または Relay 管理下の URL へ露出しない。SDK と正規 App の一時的な E2E secret handoff は Relay を経由せず、下流仕様が URL fragment 等を採用する場合も secret が Relay、HTTP request、Referer、server log、browser storage、diagnostics、analytics、telemetry、error reporting、clipboard または別経路へ流出しない。fallback 経路で必要最小限の一時処理を行う場合も、secret を DOM へ不要に露出させず、処理後に URL / browsing context から除去し、継続保持しない。malformed または受入不能な envelope structure、oversized request / response、invalid / expired lifetime、unsupported protocol / version、unauthorized credential、不正な request / session / result correlation、invalid lifecycle / state transition、duplicate、replay または stale state は、plaintext の復号・semantic interpretation・署名可否判断をせず、secret / credential / internal state を漏らさず、安全側へ拒否する。envelope 外形・size・期限・metadata・authorization・対応関係・lifecycle の transport / structural validation は、Signer の semantic validation、表示または承認の代替にならない。 |
