...
SDK と正規 Mobile App の間の一時的な E2E secret handoff は、Web Transaction Handoff Specification が定める安全条件を満たす場合に限り、App Link URL fragment を使用してよい。この fragment は Relay credential transport ではなく、Relay が E2E session secret / derived encryption material を扱うことを意味しない。handoff secret は Relay へ送信せず、HTTP request、Referer、server access log、browser storage、diagnostics、analytics、telemetry、error reporting、clipboard または fallback 経路へ露出・継続保持せず、必要な期間を越えて残存させてはならない。
