...
**MUST** 新しい handoff は、その時点の current generation context に binding されなければならない。少なくとも session / request identity と generation context が対応し、encrypted request / response の認証対象も generation context に対応しなければならない。旧 generation の identity、ciphertext または遅延配送された request は current generation の有効な handoff として扱えず、generation binding の改ざんは安全側に拒否しなければならない。generation metadata の単純な差し替えだけで、旧 ciphertext を current generation の request / response として再利用できてはならない。

state continuity を失った後は、旧 pending session を復旧せず、旧 create request、旧 request identity、旧 session identity または旧 ciphertext を current handoff として再登録・再処理してはならない。retry は fresh generation context、new request identity、new session identity、必要な新しい transport authorization context、fresh encrypted envelope および新しい利用者承認を伴う新しい handoff でなければならない。Relay はこの保証のために durable payload history、ciphertext history または利用者 handoff の長期 replay history を保持することを要求されない。

要求の有効性、使用済み判定、重複処理、遅延処理および generation の具体的な識別子、epoch、token、proof、MAC、signature、AAD field、session ID encoding、保存形式は後続仕様で一貫した安全側の規則として定義する。本要求は durable replay database、RDB、Redis AOF / RDB または persistent ciphertext digest database を必須にしない。
...
Relay が旧 state を失っていても、current generation context を境界として、旧 request identity、旧 session identity または旧 ciphertext の再利用を current generation の新しい handoff として許容してはならない。retry は fresh generation context、new identity、fresh encrypted envelope および新しい署名承認を伴うものとする。この要求は durable payload history、ciphertext history または persistent replay database を要求するものではなく、具体的な generation / epoch mechanism は後続仕様で定義する。
...
| RR-AC-003 | RR-006、RR-NFR-005                                    | replay、使用済み要求の再利用、重複配送、遅延配送、再起動後の古い状態の再出現によって追加の署名が発生しない。Relay generation を切り替えた後の旧 create request、旧 session identity、旧 request identity または同一 ciphertext は current generation の handoff として再登録・再処理されず、generation metadata の単純な差し替えも受理されない。retry は fresh generation、new identity、fresh ciphertext および新しい利用者承認を必要とする。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| RR-AC-011 | RR-006、RR-NFR-003、RR-NFR-004、RR-NFR-005            | 正常完了、利用者拒否、cancel、expiry、validation failure、timeout、Relay restart、Relay state loss その他の終端状態の後に、古い request、result、transport credential または関連 metadata が再利用できず、履歴・分析・ユーザーアカウントサービスとして保持されない。state continuity loss では旧 generation 全体が失効し、旧 create request、identity、session または ciphertext は current generation の新しい handoff として復活せず、retry は fresh generation、new identity、fresh ciphertext および新しい署名承認を必要とする。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
