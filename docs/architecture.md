...
- 通常のメッセージ署名は Origin、チェーン、ネットワーク、用途、nonce、有効期限を含む構造化形式だけを許可する。SSS 互換の従来形式は Legacy 署名として隔離する。
...
`inspectTransaction()` は単なる要約生成ではなく、署名可否を決めるセキュリティ境界である。Adapter は Product Specification 12.4 の allowlist にある transaction type / version の全フィールドと、aggregate / multisig 内の全 inner transaction を再帰的に解析する。payload は256 KiB、aggregate inner transactionは100件、ネストはouterとembeddedの2階層を上限とする。`decode(payload)` 後に canonical serialization した結果が元 payload と byte-for-byte で一致しない場合、未知 type / version、未解析フィールド、余剰 byte、上限を超えるネストまたは要素数がある場合は失敗させる。cosignature は完全な親 transaction を同時に検証できなければ失敗させる。
...
MVP の `LocalVaultSigner` は同一秘密鍵を Symbol / NEM の両 Adapter で使用する。これは NEM 側のニーモニック互換を要件とせず、Symbol Desktop Wallet 互換の派生結果を NEM SDK に入力するための意図的な設計判断である。両チェーンで同じ秘密鍵を使うため、一方の実装侵害が両 Identity に及ぶことを UI と脅威モデルへ明記し、チェーンごとに異なる署名アルゴリズム、公開鍵導出、署名入力の既知ベクトルを検証する。
...
Legacy メッセージは SSS Adapter からのみ生成でき、専用 request type、短い有効期限、Origin / tab / frame binding を持たせる。通常 Provider から Legacy 署名を呼び出す経路は設けない。
...
interface StoredEnvelope {
  ...
  vaults: Array<{
    ...
    cipher: { name: string; nonce: string; aad: string; ciphertextAndTag: string };
  }>;
  publicIndex: PublicProfileIndex[];
  settings: PublicSettings;
  permissions: PermissionGrant[];
  usedMessageNonces: Array<{
    nonceHash: string;
    origin: string;
    profileId: string;
    accountId: string;
...
dApp.sign*({ chain, network, ...params })
  ...
  → Chain Adapter で payload の全フィールドを検証・解析
  → payload の chain / network と要求値の一致を検証
  → canonical 再シリアライズ、signer、全 inner transaction、上限を検証
...
- namespace alias、mosaic name / divisibility 等をローカルで確定できない場合、推測した名称や換算額を表示しない。署名対象から検証できる raw ID と atomic amount を「外部未検証」として表示する。
...
- Legacy メッセージ署名では構造化署名でないことと再利用リスクを固定警告し、通常の構造化署名と視覚的に区別する。
...
MVP のローカル監査記録は秘密鍵、ニーモニック、password、full message、full transaction payload を含めない。次の情報を canonical record として保持またはユーザーが export できるようにする。
...
監査記録は hash chain または署名で改ざんを検知でき、保存期間、削除、暗号化 export を設定可能にする。個人向け既定では詳細監査を opt-in とし、組織向けでは管理 policy により強制できる設計とする。
...
- 全 Provider エラーコード
- SSS 対応表の各メソッド
- 通常 Provider からの Legacy メッセージ署名拒否
- SSS Legacy state の Origin / tab / frame binding、TTL、single-use
- 未対応 SSS API の安全な失敗
...
1. 対応 transaction type / version 表を固定し、全フィールド解析、canonical 再シリアライズ、未知 transaction 拒否を実装する。
...
4. Symbol Desktop Wallet の派生パスと Symbol / NEM 共用秘密鍵の chain 別署名を既知ベクトルとして固定する。
...
7. 現在 Background 内にある一時状態・権限・承認処理を Application Service へ分離し、Profile mutex と revision 検証を追加する。
...
10. SSS Legacy 署名を専用経路へ隔離し、`requestSSS()` / `isAllowedSSS()` を実権限へ接続する。
11. Provider から管理 API と Legacy message API を削除し、接続・署名引数へ chain / network を追加する。
12. CSP、RPC schema validation、fuzz test、E2E security test、SBOM / release provenance を追加する。
...
- ADR-008: 構造化メッセージ署名と SSS Legacy 署名の隔離
