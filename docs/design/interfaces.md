...
`Chain` は Symbol または NEM のような対象チェーンを示し、`Network` はそのチェーン上の Mainnet / Testnet などのネットワーク文脈を示す。両者を一つの `networkId` だけで表現することを前提にしない。Application Profile は一つの Chain と一つの Network に固定され、要求、Account、Profile、署名対象および結果は、適用される Chain と Network の組み合わせを保ったまま扱う。Symbol と NEM の両方を利用する場合は、Chain ごとに別 Profile-local context を使用する。
...
Application Profile と、その Profile に固定された Profile Chain / Network は、公開 request / response field ではない Signer-local な security context とする。Profile ID を SDK、Relay またはその他の公開 wire schema に追加することは要求しない。ここでいう Application Profile は、wallet-core の Profile / Store と同一の責任単位ではなく、wallet-core の内部 Profile / Store を Application Profile の authority として扱わない。

Application Account、permission、active Account、approval および signing authorization は、所属 Profile の固定された Chain / Network と一致しなければならない。異なる Chain の Account、permission または authorization を同一 Profile-local context へ関連付けず、両 Chain を利用する場合は Profile-local context 自体を分離する。
...
| Application / Signer       | Application Profile における Account association、Account selection、display、permission、Account authorization および active Profile / Account context。Account / permission は Profile の固定 Chain / Network と一致させる。 |
...
- **Profile-local security context**: Application Profile、固定 Profile Chain / Network、active Profile / Account association および Signer-local な permission / authorization context。公開 request field ではない。
...
- **handoff participant / recipient context**: 適用される handoff における intended recipient / participant、必要な device または Signer-local identity、handoff session / generation、および response channel / direction。公開 field の追加を意味せず、request / response の相関と recipient の正当性を確認するための概念 context とする。
...
- 適用される handoff の intended recipient / participant、device または Signer-local identity、session / generation、response channel / direction、および response delivery context。
...
#### Cancellation と terminal outcome

Cancellation は SigningRequest に対する lifecycle operation であり、同じ request identity と、適用される caller / source、session / generation、recipient / participant、channel / direction、operation、target および freshness の binding context に対してだけ発行・検証する。cancel の発行主体は、該当する lifecycle boundary の authority（利用者、dApp、Signer、platform または transport）に限り、SDK の local wait 終了だけでは Signer-side cancellation completion を意味しない。具体的な API、ack field および transport は下位仕様へ委譲する。

Cancel の共通結果は、少なくとも次の意味を失わない。cancel request の受理、拒否または既に terminal であることを、同じ request identity に対する acknowledgement / terminal response として扱う。署名開始前に cancel が受理され、署名を開始していないことを Signer が確定できる場合は `CANCELLED` とする。`SIGNING` と競合し、署名生成自体の成否を確定できない場合は `RESULT_UNKNOWN` とし、署名が生成されていないと推測しない。署名生成の不成立を確定できる場合だけ `CANCELLED` とする。既知の署名成功後の cancel は既存 result を取り消さず、response delivery が不明なら `SUCCEEDED` と `DELIVERY_UNKNOWN` を維持する。expiry、timeout、user rejected および確定 failure は cancel と自動変換しない。

`CANCELLED`、`RESULT_UNKNOWN`、`SUCCEEDED + DELIVERY_UNKNOWN` およびその他の terminal outcome は相互に潰さず、terminal state を reopen、同じ request の再認証・再署名または automatic re-sign の根拠にしない。
...
- cancel request / acknowledgement と、署名前・`SIGNING` 中・既知成功後の outcome precedence。Cancel の具体 API、wire schema および concurrency は下位仕様へ委譲する。
- handoff の intended recipient / participant、device または Signer-local identity、session / generation、response channel / direction の request / response binding。recipient、device、channel、generation または direction の不一致は fail-closed とする。
...
| cancel request の request identity、scope、authority および acknowledgement                            | Signer と該当 lifecycle boundary     | cancel の受理・拒否・既 terminal を元 request に対応付け、署名前 / `SIGNING` 中 / 既知成功後の outcome を混同しない                                                                          |
| browser caller context                                                                                 | Browser Extension privileged layer   | browser が観測した Origin / sender / document と request の対応確認                                                                                                                          |
| Mobile caller / handoff context                                                                        | Mobile App                           | handoff metadata、session、完全性、期限、request source、intended recipient / participant、device / Signer identity および response channel / direction の確認                               |
| request identity、version、operation、permission、freshness                                            | Signer                               | 署名判断の request binding、Profile-local context、未知・重複・期限切れの拒否                                                                                                                |
| Profile-local security context                                                                         | Application / Signer                 | active Profile、固定 Profile Chain / Network、Profile / Account association、permission および lifecycle の継続確認                                                                          |
...
| wallet-core result と response                                                                         | Signer                               | signature / signed payload と承認済み target、Profile、Account、Chain / Network、operation、4条件、correlation、intended recipient、channel / direction および response recipient の対応確認 |
...
- intended recipient / participant、device または Signer identity、session / generation、response channel / direction が request / response binding と一致しない場合は、delivery success にかかわらず fail-closed で終端する。Relay / transport の ACK は approval、署名成功または delivery completion の代替ではない。
...
| 本書で固定する設計内容                                                                                  | 上位・関連資料                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Profile-local context、Profile Chain / Network、lifecycle invalidation、Browser / Mobile caller binding | [Architecture](./architecture.md) §6.6 / §6.9、[Security Design](./security-design.md) §6 / §7 / §9 / §10、[Signing Flow](./signing-flow.md) §5 / §7 / §16 / §20、[Browser Extension Design](./browser-extension.md) §7 / §12 / §17、[Mobile App Design](./mobile-app.md) §7 / §9 / §14 / §21                                                                                                                                                                  |
...
| failure semantics、cancellation outcome、`RESULT_UNKNOWN` / `DELIVERY_UNKNOWN`、automatic re-sign 禁止  | [Requirements](../requirements/requirements.md) `CR-012` / `CR-NFR-010`〜`CR-NFR-012`、[Signing Flow](./signing-flow.md) §7、§20〜§22、[Interfaces Specification](../specifications/interfaces.md) §10、[Signing Protocol](../specifications/signing-protocol.md) §19 / §20                                                                                                                                                                                    |
| handoff recipient / participant、device / Signer identity、response channel / direction の binding      | [Requirements](../requirements/requirements.md) `CR-006`、`CR-007`、`CR-NFR-011`、[Relay Requirements](../requirements/relay.md) `RR-001`〜`RR-009`、[Signing Flow](./signing-flow.md) §5、§7、§19〜§20、[Relay Specification](../specifications/relay.md) §6〜§9                                                                                                                                                                                              |
