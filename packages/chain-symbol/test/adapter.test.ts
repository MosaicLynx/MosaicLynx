...
import { deriveSharedAccount, SymbolChainAdapter } from "../src/index.js";
...
describe("SymbolChainAdapter", () => {
  ...
  it("strictly inspects, signs, and verifies a canonical Transfer v1", () => {
    ...
    expect(new SymbolFacade("testnet").verifyTransaction(
      signedTransaction,
      new Signature(signedTransaction.signature.bytes),
    )).toBe(true);
