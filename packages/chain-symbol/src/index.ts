...
const bytesFor = (payload: string): Uint8Array => {
  ...
  if (bytes.length > MAX_PAYLOAD_BYTES)
    throw new Error("INVALID_TRANSACTION: payload exceeds 256 KiB");
  return bytes;
};
...
const inspect = (
  ...
  try { transaction = SymbolTransactionFactory.deserialize(bytes); }
  catch { throw new Error("INVALID_TRANSACTION: Symbol payload cannot be decoded"); }
  ...
  if (transaction instanceof models.TransferTransactionV1) {
  ...
  } else if (transaction instanceof models.AggregateCompleteTransactionV2
    || transaction instanceof models.AggregateBondedTransactionV2) {
    schema = transaction instanceof models.AggregateCompleteTransactionV2
      ? "AggregateCompleteTransactionV2" : "AggregateBondedTransactionV2";
    ...
    for (const cosignature of transaction.cosignatures as Array<{ version: bigint; signerPublicKey: { toString(): string } }>) {
