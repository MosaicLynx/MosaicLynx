...
import { isSupportedApiVersion, RpcMosaicLynxProvider, type RpcExecutor } from "../src/index.js";

describe("Provider API v2", () => {
  it("exposes only dApp-safe Promise methods through request RPC", async () => {
    const requests: unknown[] = [];
    const executor: RpcExecutor = { request: async (request) => { requests.push(request); return []; } };
