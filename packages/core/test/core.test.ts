...
class MemoryProfiles implements ProfileRepository {
  ...
  public async save(profile: Profile): Promise<void> { this.records.set(profile.id, profile); }
  public async getById(id: string): Promise<Profile | undefined> { return this.records.get(id); }
...
class MemoryAccounts implements AccountRepository {
  ...
  public async save(account: Account): Promise<void> { this.records.set(account.id, account); }
  public async getById(id: string): Promise<Account | undefined> { return this.records.get(id); }
  ...
  public async remove(id: string): Promise<void> { this.records.delete(id); }
}
...
class MemoryPermissions implements PermissionRepository {
  ...
  public async get(origin: string, profileId: string, scope: PermissionGrant["scope"]): Promise<PermissionGrant | undefined> {
...
describe("Core profile, account, permission and message boundaries", () => {
  ...
  it("stores network at profile level and never reuses account indexes", async () => {
    ...
    const profile = await new ProfileService(profiles, clock, ids).create(
      "mainnet", "Personal", "account-0", "vault-1",
    );
  ...
  it("scopes permissions by origin, profile, chain, network and account set", async () => {
    ...
    await expect(permissions.assertConnected("https://example.com", "profile-1", mainnet))
      .resolves.toMatchObject({ accountIds: ["account-1"], revision: 1 });
    await expect(permissions.assertConnected(
      "https://example.com", "profile-1", createChainScope("nem", "testnet"),
  ...
  it("locks vaults independently by profile", async () => {
    ...
    const vault: ProfileVaultPort = {
      unlock: async (profileId) => { locked.delete(profileId); return true; },
      lock: async (profileId) => { locked.add(profileId); },
      isLocked: async (profileId) => locked.has(profileId),
    };
    await expect(new VaultSessionService(vault).assertUnlocked("profile-1"))
  ...
  it("domain-separates and validates structured messages", () => {
    const { message, signingBytes } = createStructuredMessage("https://example.com/path", {
      chain: "symbol",
      network: "testnet",
      purpose: "login:v1",
