...
export class AccountService {
  ...
  public async add(account: Account): Promise<void> {
    ...
    if (!account.name.trim() || !account.identities.symbol.address || !account.identities.nem.address
    ...
    if (account.source.kind === "mnemonicDerived"
      && (account.source.accountIndex < 0 || account.source.accountIndex >= 2 ** 31
        || account.source.accountIndex >= profile.nextAccountIndex))
      throw new MosaicLynxError("PROFILE_SCOPE_MISMATCH", "Account derivation index is outside the profile allocation.");
  ...
  public getActive(profileId: string): Promise<Account> { return this.getDefault(profileId); }

  public async remove(profileId: string, accountId: string): Promise<void> {
    const profile = await this.profiles.getById(profileId);
    if (!profile) throw new MosaicLynxError("PROFILE_NOT_FOUND", "Profile was not found.");
    if (!profile.accountIds.includes(accountId)) throw new MosaicLynxError("ACCOUNT_NOT_FOUND", "Account was not found.");
    if (profile.accountIds.length === 1) throw new MosaicLynxError("LAST_ACCOUNT", "The last account cannot be deleted.");
...
export class PermissionService {
  ...
  public async grant(
    ...
    try { canonicalOrigin = new URL(origin).origin; } catch { throw new MosaicLynxError("INVALID_PARAMS", "Origin is invalid."); }
  ...
  public async assertConnected(origin: string, profileId: string, scope: ConnectionScope): Promise<PermissionGrant> {
    ...
    if (!grant) throw new MosaicLynxError("UNAUTHORIZED_ORIGIN", "This origin is not connected for the requested scope.");
    return grant;
  }
}
...
export class VaultSessionService {
  ...
  public lock(profileId: string): Promise<void> { return this.vault.lock(profileId); }
...
export class SigningService {
  ...
  public async signMessage(input: {
    profileId: string; accountId: string; signingBytes: Uint8Array; recipientPublicKey?: string;
  }): Promise<string> {
  ...
  public async signTransaction(input: {
    profileId: string; accountId: string; chain: ConnectionScope["chain"]; payload: string;
