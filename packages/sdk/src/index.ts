...
const fail = (code: MosaicLynxSDKErrorCode): MosaicLynxSDKError =>
...
const isProviderShape = (value: unknown): value is MosaicLynxProvider => {
  ...
  return typeof provider?.apiVersion === "string"
...
const providerState = (): "none" | "supported" | "unsupported" => {
  ...
  return isProviderShape(window.mosaicLynx)
    && isSupportedApiVersion(window.mosaicLynx.apiVersion)
    ? "supported"
    : "unsupported";
};
...
const hasMobilePlatform = (): boolean => {
  ...
  if (!globalThis.crypto?.subtle || typeof fetch !== "function" || typeof document.visibilityState !== "string") return false;
...
const mapProviderError = (error: unknown): MosaicLynxSDKError => {
  ...
  const allowed: readonly MosaicLynxSDKErrorCode[] = [
    "USER_REJECTED", "VAULT_LOCKED", "INVALID_PARAMS", "INVALID_TRANSACTION",
    "UNSUPPORTED_TRANSACTION", "CHAIN_MISMATCH", "NETWORK_MISMATCH",
    "CONTEXT_CHANGED", "REQUEST_EXPIRED",
  ];
  return fail(allowed.includes(code as MosaicLynxSDKErrorCode)
    ? code as MosaicLynxSDKErrorCode
    : code === "ACCOUNT_NOT_FOUND" ? "SIGNER_MISMATCH"
      : code === "UNSUPPORTED_CHAIN" ? "UNAVAILABLE"
        : "INTERNAL_ERROR");
};
...
): readonly MosaicAccount[] => accounts.filter((account) =>
  account.scope.chain === params.chain && account.scope.network === params.network);
...
const isOriginProof = (value: unknown): value is OriginProof => {
  ...
  return proof?.version === "mosaiclynx.origin.v1"
    && proof.algorithm === "Ed25519"
    && typeof proof.keyId === "string" && proof.keyId.length > 0
    && typeof proof.signature === "string" && /^[A-Za-z0-9_-]+$/.test(proof.signature);
};
...
const signWithMobileRelay = async (
  params: MosaicLynxSignTransactionParams,
): Promise<SignedTransaction> => {
  ...
  openAppLink(`https://link.mosaiclynx.app/v1/handoff/${sessionId}#s=${base64UrlEncode(sessionSecret)}&a=${appToken}`);
  ...
  try {
    ...
    while (Date.now() < created.getTime() + 5 * 60_000) {
      ...
      const result = logical as {
        protocol?: unknown; requestId?: unknown; requestDigest?: unknown; outcome?: unknown;
        signedTransaction?: SignedTransaction; errorCode?: MosaicLynxSDKErrorCode;
      };
      ...
      if (result.outcome === "rejected" || result.outcome === "failed") {
        const responseCodes: readonly MosaicLynxSDKErrorCode[] = [
          "USER_REJECTED", "VAULT_LOCKED", "REQUEST_EXPIRED", "INVALID_PARAMS",
          "INVALID_TRANSACTION", "UNSUPPORTED_TRANSACTION", "CHAIN_MISMATCH",
          "NETWORK_MISMATCH", "SIGNER_MISMATCH", "CONTEXT_CHANGED", "INTERNAL_ERROR",
        ];
        const code = result.outcome === "rejected"
          ? "USER_REJECTED"
      ...
      try { verified = verifySignedTransaction(params, result.signedTransaction); }
...
class DefaultMosaicLynxSDK implements MosaicLynxSDK {
  ...
  public async signTransaction(params: MosaicLynxSignTransactionParams): Promise<SignedTransaction> {
    ...
    try {
      ...
      const normalized = error instanceof MosaicLynxSDKError
        ? error
        : error instanceof TypeError ? fail("INVALID_PARAMS") : fail("INTERNAL_ERROR");
  ...
  private validate(params: MosaicLynxSignTransactionParams): void {
    if (!params || (params.chain !== "symbol" && params.chain !== "nem")
...
export const createMosaicLynxSDK = (
  options: MosaicLynxSDKOptions = {},
): MosaicLynxSDK => new DefaultMosaicLynxSDK(options);
