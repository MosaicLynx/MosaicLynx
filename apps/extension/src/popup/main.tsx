...
const App = () => {
  ...
  const candidates = useMemo(
    () =>
      normalizeMnemonic(mnemonic)
        ...
        .sort((a, b) => a.word.localeCompare(b.word) || a.id - b.id),
    [mnemonic]
  );
  ...
  if (!profile || creating) {
    ...
    const header = (title: string, body: string) => (
      <header className="flow-header">
        <div className="brand-row">
          <h1>MosaicLynx</h1>
          {flowStep && <span className="step-count">{flowStep} / 4</span>}
        </div>
    ...
    const backButton = (onClick: () => void) => (
      <Button variant="outlined" color="inherit" type="button" onClick={onClick}>
        {t('back')}
      </Button>
    );
      ...
      <main className="app-shell onboarding">
        {step === 'welcome' && isFirstProfile && (
          <>
            ...
            <footer>
              <button className="primary wide" onClick={() => setStep('method')}>
                {t('start')}
              </button>
            </footer>
          </>
          ...
          <>
            ...
            <section className="method-list">
              ...
              >
                ...
                <span>{t('importBody')}</span>
                <b aria-hidden="true">→</b>
              </button>
            </section>
            <footer>
              {isFirstProfile ? (
                backButton(() => setStep('welcome'))
              ) : (
                <button onClick={cancelCreation}>{t('cancel')}</button>
              )}
            </footer>
          </>
        )}
        ...
        {step === 'details' && (
          <>
            ...
            <section className="form-grid">
              ...
              {network === 'mainnet' && !MAINNET_SIGNING_ENABLED && (
                <p className="warning-panel inline-panel">{t('mainnetDisabled')}</p>
              )}
              <PasswordField
                label={t('password')}
                value={password}
            ...
            <footer>
              ...
              <button className="primary" onClick={continueFromDetails}>
                {t('next')}
              </button>
            </footer>
          </>
        ...
        {step === 'import' && (
          <>
            ...
            <footer>
              ...
              <button className="primary" onClick={reviewImport}>
                {t('next')}
              </button>
            </footer>
          </>
        ...
        {step === 'import-review' && importPreview && (
          <>
            ...
            <footer>
              ...
              <button className="primary" disabled={busy} onClick={() => void createProfile()}>
          ...
          <>
            ...
            <section className="word-confirmation">
              <div className="word-label">
                <span>
                  {t('selectedWords')} ({selectedWordIds.length}/24)
                </span>
                <span>
                  <button
                    className="text-button"
                    disabled={!selectedWordIds.length}
                    onClick={() => setSelectedWordIds(selectedWordIds.slice(0, -1))}
                  >
                    {t('undo')}
                  </button>
                  <button
              ...
              <div className="word-candidates">
                {candidates.map((candidate) => (
                  <button
                    key={candidate.id}
                    disabled={selectedWordIds.includes(candidate.id)}
                    onClick={() => setSelectedWordIds([...selectedWordIds, candidate.id])}
                  >
                    {candidate.word}
                  </button>
                ))}
        ...
        {step === 'complete' && (
          <>
            <div className="welcome-mark complete-mark" aria-hidden="true">
              ✓
            </div>
  ...
  const pageHeader = (title: string, backTo: HomeView = 'menu') => (
    <header className="page-header">
      <button className="icon-button" type="button" aria-label={t('back')} onClick={() => openView(backTo)}>
        ←
      </button>
      <h1>{title}</h1>
    </header>
        ...
        <section className="menu-list">
          <button className="menu-item" onClick={() => openView('accounts')}>
            <span>
              ...
              <small>{t('accountCount', { count: profileAccounts.length })}</small>
            </span>
            <b>›</b>
          </button>
        ...
        <section className="menu-settings">
          ...
          <div className="setting-row">
            ...
            <ToggleButtonGroup
              ...
              onChange={(_, value: ExtensionStore['settings']['theme'] | null) => {
                if (value) void updateSettings({ theme: value });
              }}
              aria-label={t('theme')}
            >
              <ToggleButton value="light" aria-label={t('lightTheme')}>
                <LightModeOutlined fontSize="small" />
              </ToggleButton>
              <ToggleButton value="dark" aria-label={t('darkTheme')}>
                <DarkModeOutlined fontSize="small" />
              </ToggleButton>
            </ToggleButtonGroup>
          </div>
          <p className="assurance">
            <strong>{t('softwareVault')}</strong>
            <br />
            {t('assurance')}
          </p>
        </section>
          ...
          {store.profiles.map((item) => (
            ...
            >
              <span>
                ...
                <small>{item.network.toUpperCase()}</small>
              </span>
      ...
      <main className={`app-shell management-shell theme-${store.settings.theme}`}>
        ...
        {showAddAccount ? (
          ...
          <section className="edit-panel account-edit-panel">
            <label className="field">
              <span>{t('accountName')}</span>
              <input value={accountNameDraft} onChange={(event) => setAccountNameDraft(event.target.value)} />
            </label>
            {error && (
              <p className="form-error" role="alert">
                {error}
            ...
            <div className="button-row">
              <button
                ...
                onClick={() => account && setConfirmationAction({ kind: 'account', name: account.name })}
              >
                {t('deleteAccount')}
              </button>
              <button className="primary" onClick={() => void renameAccount()}>
                {t('rename')}
              </button>
            </div>
          </section>
    ...
    <main className={`app-shell home-shell theme-${store.settings.theme}`}>
      <header className="home-header">
        <div className="home-title-row">
          <h1>MosaicLynx</h1>
          <button className="icon-button menu-button" aria-label={t('menu')} onClick={() => openView('menu')}>
            <span></span>
            <span></span>
            <span></span>
          </button>
