...
const App = () => {
  ...
  const approve = async (): Promise<void> => {
    ...
    try {
      ...
      if (!envelope || envelope.revision !== approval.vaultRevision) throw new ApprovalError('approvalProfileChanged');
  ...
  if (!approval)
    return (
      <main className="app-shell">
        <p className="empty-state">{t('approvalRequestUnavailable')}</p>
      </main>
    );
  ...
  return (
    <main className="app-shell approval-shell">
      <header className="approval-header">
        <div className="approval-brand-row">
          <h1>MosaicLynx</h1>
          <Chip
            size="small"
            color={approval.scope.network === 'mainnet' ? 'error' : 'secondary'}
            label={approval.scope.network.toUpperCase()}
          />
        </div>
        <p className="eyebrow">
          {t(approvalTypeKey[approval.type])} · {approval.scope.chain.toUpperCase()}
        </p>
        <p className="origin">
          <strong>{approval.origin}</strong>
          <br />
          <span>{approval.originAscii}</span>
        </p>
      </header>
      ...
      <Card
        ...
        aria-label={t('approvalRequestSummary')}
      >
        {summary.map((item) => (
          ...
          {approval.availableAccounts.map((account) => (
            <label className="account-choice" key={account.id}>
              <Checkbox
                ...
                onChange={(event) =>
                  setSelected(
                    event.target.checked ? [...selected, account.id] : selected.filter((id) => id !== account.id)
                  )
                }
              />
              <span>
      ...
      <footer>
        <Button fullWidth variant="outlined" color="inherit" type="button" onClick={reject} disabled={busy}>
          {t('approvalReject')}
        </Button>
        <Button
          fullWidth
